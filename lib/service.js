...
(function(){
    ...
    var scanDevice = function(devices, that){                
        for (var i=0; i<devices.length; i++)
        {
            ...
            console.log('Media-Play API: UPnP scan hits timeout finding ' + devices.length + ' device(s).');
            ...
            if(!that.registeredModules[devices[i].uuid])
            {
                ...
                console.log("Media-play API: XBMC implementation has been loaded for device " + devices[i].friendlyName + " at " + devices[i].ip);
            }
        }
        for (var i in that.registeredModules)
        {
            var found = false;
            ...
            if(!found && typeof that.registeredModules[i] !== 'function'){
                that.unregister(that.registeredModules[i]);
                console.log("Media-play API: unregistering " + i);
                
                that.registeredModules[i].connection.close();                
                delete that.registeredModules[i].connection;                
                delete that.registeredModules[i];    
            }
        }

        setTimeout(upnpScan.scan, 30000, 5000, that, scanDevice);
    }
    ...
    MediaApiModule.prototype.init = function(register, unregister)
    {
        ...
        if(process.platform === "linux" || process.platform === "darwin"){
            ...
            players.omx.on("close", function(exitCode){
                ...
                else if(players.mplayer.exitCode !== 0) //should never be called
                    console.log("Media-play API: supported players not found on this system");
            });
        }
